...
class PubGeometry():
    def __init__(self):
        ...
        rospy.init_node('pub_dummy', anonymous=True)

        image_topic = rospy.get_param('~image', '/tachyon/image')
        rospy.Subscriber(image_topic, Image, self.callback, queue_size=5)
    ...
    def callback(self, data):
        ...
        try:
            ...
            if cnt is not None:
                ...
                msg_text = "Data: %s,%s,%s " % (angle_rads, major_axis, minor_axis)
                rospy.loginfo(msg_text)

                self.msg_geo.header.stamp = rospy.Time.now()
